...
export async function generateMetadata({
  ...
  const title = `${company.name} — ${company.sectorLabel}, ${company.city}`;
  const description = `${company.name} ${company.city} ${company.district ?? ""} adresinde ${company.sectorLabel} hizmeti veriyor. WhatsApp ile ${company.phone} numarasından ulaşabilirsiniz. ${company.about.slice(0, 100)}`;
  ...
  return {
    ...
    openGraph: {
      ...
      images: company.coverImageUrl
        ? [{ url: company.coverImageUrl, width: 1200, height: 630, alt: company.name }]
        : undefined,
    },
    twitter: {
...
function CompanyProfileView({ company }: { company: Company }) {
  ...
  const jsonLd = {
    ...
    url: company.website ?? `https://firma.protakip.com/${company.slug}`,
  };
          ...
          <div className="bg-white rounded-2xl shadow-xl shadow-zinc-900/5 border border-zinc-200 overflow-hidden">
            ...
            <div className="p-6 md:p-8 border-b border-zinc-100">
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 w-full mt-3 px-6 py-3 rounded-xl bg-zinc-100 hover:bg-zinc-200 text-zinc-900 font-semibold transition-colors"
              >
