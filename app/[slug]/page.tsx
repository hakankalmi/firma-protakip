...
import { formatPhoneForDisplay, resolveTelephone } from "@/lib/phone";
import {
...
import { cleanAddress, displayPlace } from "@/lib/text";
...
export async function generateStaticParams() {
  ...
  return slugs.slice(0, PRERENDER_LIMIT).map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  if (!company) {
    return { title: "Firma bulunamadı", robots: { index: false, follow: false } };
  }
  ...
  const logo = isHttpUrl(company.logoUrl) ? company.logoUrl : null;
...
function CompanyProfileView({ company }: { company: Company }) {
  const rich = isRichProfile(company);
  const waLink = buildWhatsAppLink(company);
  const telephone = resolveTelephone(
    company.phone,
      ...
      {rich && (
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: serializeJsonLd(buildLocalBusinessJsonLd(company)),
          }}
        />
      )}
        ...
        <div className="max-w-3xl mx-auto px-4 -mt-16 md:-mt-20 pb-16">
          <div className="bg-white rounded-2xl shadow-xl shadow-zinc-900/5 border border-zinc-200 overflow-hidden">
            ...
            <div className="p-6 md:p-8 border-b border-zinc-100">
              <nav
                aria-label="Gezinti"
                className="flex flex-wrap items-center gap-1.5 text-xs text-zinc-500 mb-4"
              >
              ...
              {telephone && (
                ...
                >
                  ...
                  {formatPhoneForDisplay(telephone)}
                </a>
              )}
            </div>
            ...
            {address && mapsHref && (
              <Section title="Adres">
                <p className="text-zinc-700 leading-relaxed">{address}</p>
                <a
                  href={mapsHref}
                  target="_blank"
                  rel="noopener noreferrer"
